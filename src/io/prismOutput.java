...
public class prismOutput {
	...
	public void addTable(String titleTable, ArrayList<String> titleColumn, ArrayList<String[]> data){
		...
		String[] columnsStarts=new String[]{"<XColumn Width=\"81\" Decimals=\"1\" Subcolumns=\"1\">\n", "<YColumn Width=\"183\" Decimals=\"3\" Subcolumns=\"1\">\n"};
		String[] columnsEnds=new String[]{"</XColumn>\n", "</YColumn>\n"};
		
		/*out+=	 "<RowTitlesColumn Width=\"89\">\n"
				+"<Subcolumn></Subcolumn>\n"
				+"</RowTitlesColumn>"; // In case we explicitly need to set the row titles to empty*/
		
		for(int i=0; i<titleColumn.size(); i++){
			out+=	columnsStarts[i==0?0:1]
					+"<Title>"+titleColumn.get(i)+"</Title>\n"+
					"<Subcolumn>\n";
			
			for(int j=0; j<data.size(); j++)out+="<d>"+data.get(j)[i].replace(".", ",").replace(">", "-").replace("<", "-")+"</d>\n";
